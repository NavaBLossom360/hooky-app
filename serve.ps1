...
param([int]$Port = 8787)
$root = $PSScriptRoot
$mime = @{ ".html"="text/html; charset=utf-8"; ".js"="text/javascript; charset=utf-8"; ".css"="text/css; charset=utf-8"; ".json"="application/json"; ".png"="image/png"; ".jpg"="image/jpeg"; ".svg"="image/svg+xml"; ".md"="text/plain; charset=utf-8" }
...
while ($listener.IsListening) {
  ...
  $res = $ctx.Response
  # Dev-only upload endpoint used by logo.html to save generated PNGs into assets/.
  if ($ctx.Request.HttpMethod -eq "POST" -and $ctx.Request.Url.AbsolutePath -eq "/upload") {
    $name = $ctx.Request.QueryString["name"]
    if ($name -match "^[a-z0-9-]+.png$") {
      $dir = Join-Path $root "assets"; if (-not (Test-Path $dir)) { New-Item -ItemType Directory $dir | Out-Null }
